...
function getStatusMeta(reminderStatus, nextServiceDate) {
  ...
  if (isCompleted) {
    return {
      label: "Completed",
      style:
        "text-emerald-700 bg-emerald-50 border border-emerald-200 dark:text-emerald-400 dark:bg-emerald-950/50 dark:border-emerald-900",
      dot: "bg-emerald-500",
    };
  }
  ...
  if (days < 0) {
    return {
      ...
      style:
        "text-rose-700 bg-rose-50 border border-rose-200 dark:text-rose-400 dark:bg-rose-950/50 dark:border-rose-900",
  ...
  if (days <= 7) {
    return {
      label: "Due Soon",
      style:
        "text-amber-700 bg-amber-50 border border-amber-200 dark:text-amber-400 dark:bg-amber-950/50 dark:border-amber-900",
  ...
  function StatusBadge({ reminderStatus, nextServiceDate }) {
    const { label, style, dot } = getStatusMeta(
      reminderStatus,
      nextServiceDate,
    );
    return (
  ...
  function ReminderCard({ r, onSendEmail, onSendSMS, onCall, isSending }) {
    ...
    const isCompleted =
      r.reminderStatus === "Completed" && nextDateNorm <= today;
    ...
    const startDate =
      lastDateNorm && today < lastDateNorm ? lastDateNorm : today;
              ...
              <button
                ...
                className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 text-[12px] font-bold rounded-xl transition active:scale-95 ${
                  isSending
                    ...
                    : "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50! border border-blue-200 dark:border-blue-900 hover:bg-blue-600 dark:hover:bg-blue-600! hover:text-white"
                }`}
              >
                <Mail size={14} />
            ...
            {r.customerId?.phone && (
              <>
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-[12px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50! border border-emerald-200 dark:border-emerald-900 rounded-xl hover:bg-emerald-600 dark:hover:bg-emerald-600! hover:text-white transition active:scale-95"
                >
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-[12px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-900 rounded-xl hover:bg-indigo-600 dark:hover:bg-indigo-600! hover:text-white transition active:scale-95"
                >
                  <Phone size={14} />
                  Call
                </a>
  ...
  export default function ServiceReminders() {
    ...
    const exportColumns = [
      ...
      },
      {
        header: "Customer",
        accessor: (row) =>
          row.customerId?.name || row.customerName || "Walk-in",
      },
      ...
      { header: "Email", accessor: (row) => row.customerId?.email || "N/A" },
      {
        ...
        accessor: (row) =>
          row.lastServiceDate
            ? format(new Date(row.lastServiceDate), "dd MMM yyyy")
            : "N/A",
      },
      {
        ...
        accessor: (row) =>
          row.nextServiceDate
            ? format(new Date(row.nextServiceDate), "dd MMM yyyy")
            : "N/A",
      },
      {
        header: "Status",
        accessor: (row) =>
          getStatusMeta(row.reminderStatus, row.nextServiceDate).label,
      },
    ];
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-3 bg-blue-600 dark:bg-blue-950/90 hover:bg-blue-700 rounded-2xl text-sm font-bold text-white transition-all duration-300 shadow-sm hover:shadow-md disabled:opacity-70 h-11"
              >
                <svg
