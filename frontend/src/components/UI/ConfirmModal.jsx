...
export default function ConfirmModal({ 
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="sm" isError={type === 'danger' || type === 'error'}>
      <div className="p-4 sm:p-6 md:p-8 flex flex-col items-center text-center">
        {/* Icon */}
        <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mb-6 ${iconColors[type]}`}>
        ...
        <div className="flex w-full gap-4">
          ...
          <button
            onClick={onConfirm}
            className={`flex-1 px-6 py-4 text-white font-bold rounded-2xl transition-all shadow-lg hover:-translate-y-0.5 active:translate-y-0 ${buttonColors[type]}`}
          >
            {confirmText}
          </button>
        </div>
